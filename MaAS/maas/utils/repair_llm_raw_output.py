...
def repair_llm_raw_output(output: str, req_keys: list[str], repair_type: RepairType = None) -> str:
    ...
    typical case
        ...
        3. json format
            ...
            output: { xxx }]
    """
    if not getattr(config, "repair_llm_output", False):
        return output
...
def run_after_exp_and_passon_next_retry(logger: "loguru.Logger") -> Callable[["RetryCallState"], None]:
    def run_and_passon(retry_state: RetryCallState) -> None:
        ...
        if retry_state.outcome.failed:
            ...
            fix_str = "try to fix it, " if getattr(config, "repair_llm_output", False) else ""
            logger.warning(
...
_STOP_REPAIR = 3 if getattr(config, "repair_llm_output", False) else 0


@retry(
    stop=stop_after_attempt(_STOP_REPAIR),
    wait=wait_fixed(1),
