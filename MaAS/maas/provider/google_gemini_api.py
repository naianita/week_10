...
try:
    ...
    from google.ai import generativelanguage as glm  # type: ignore[import]
    from google.generativeai.generative_models import GenerativeModel  # type: ignore[import]
    from google.generativeai.types import content_types  # type: ignore[import]
    from google.generativeai.types.generation_types import (  # type: ignore[import]
    ...
    # Make Gemini provider optional so that environments without google packages
    # can still import MaAS as long as GeminiLLM is not used.
    genai = None
    glm = None

    class _Dummy:  # minimal stand‑in for type hints
        pass

    GenerativeModel = _Dummy  # type: ignore[assignment]
    content_types = _Dummy  # type: ignore[assignment]
    AsyncGenerateContentResponse = _Dummy  # type: ignore[assignment]
    BlockedPromptException = Exception  # type: ignore[assignment]
    GenerateContentResponse = _Dummy  # type: ignore[assignment]
    GenerationConfig = _Dummy  # type: ignore[assignment]
...
class GeminiLLM(BaseLLM):
    ...
    def __init__(self, config: LLMConfig):
        if genai is None:
            raise RuntimeError(
                "google-generativeai is not installed. Install it if you need the "
                "Gemini provider, or use another LLM api_type (e.g. openai)."
            )
