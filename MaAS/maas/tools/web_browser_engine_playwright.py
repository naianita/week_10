...
try:
    # Playwright is optional; MaAS GSM8K benchmark does not require it.
...
class PlaywrightWrapper(BaseModel):
    ...
    async def run(self, url: str, *urls: str) -> WebPage | list[WebPage]:
        if async_playwright is None:
            raise RuntimeError(
                "playwright is not installed. Install it and its browsers if you "
                "need web browsing tools, or avoid using Playwright-based tools."
            )
